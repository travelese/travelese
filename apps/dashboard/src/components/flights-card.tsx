...
import React from "react"; // Add this import statement at the top
...
const IncludedItems: React.FC<IncludedItemsProps> = React.memo(
  ({ segments }) => {
    ...
    > = {
      carry_on: { icon: Icons.Backpack, label: "Carry-on Bag" },
      checked: { icon: Icons.Briefcase, label: "Checked Bag" },
    };

    const amenityIcons: Record<string, React.ElementType> = {
      seat_pitch: Icons.SeatPitch,
      seat_type: Icons.SeatRecline,
      wifi: Icons.Wifi,
      power: Icons.BatteryCharging,
    };
            ...
            <div className="flex flex-col items-center justify-center w-full sm:w-1/3 mt-4 sm:mt-0 space-y-3">
              <Alert className="w-full sm:w-auto text-center">
                <div className="flex flex-col items-center">
                  <Icons.CloudFog className="h-4 w-4 mb-1" aria-hidden="true" />
            ...
            <ScrollArea className="h-[500px]">
              ...
              <CardContent className="space-y-4 pt-2">
                {outbound.segments.map((segment, index) => (
                  <FlightsSegment key={index} segment={segment} />
                ))}
              </CardContent>
            </ScrollArea>
          </Card>
...
const FlightsSegment: React.FC<{ segment: OfferSliceSegment }> = ({
  segment,
}) => {
  const {
    aircraft,
        ...
        <AccordionItem value="item-1">
          <AccordionTrigger>
            <div className="flex items-center">
              <Icons.Flights className="w-5 h-5 mr-2" />
    ...
    <div className="flex flex-col items-center">
      ...
      {distance && (
        <div className="text-xs mt-1">
          {Number.parseFloat(distance).toLocaleString(undefined, {
...
FlightsCard.displayName = "FlightsCard";
