...
export async function travelAgent(prompt: string) {
  ...
  (async () => {
    const { partialObjectStream } = await streamObject({
      model: xai("grok-beta"),
      system: `You are an AI travel agent that converts natural language into structured travel requests.
            IMPORTANT RULES:
            1. For flights:
            - Flight Arrangements: Detail user preferences for flight bookings, including origin, destination, travel class preferences, and additional requirements like layover times or specific airlines. Utilize real-time data for optimal availability and pricing.
            ...
            2. For stays:
            - Accommodation Bookings: For stays, specify desired location, type of accommodation, check-in and check-out dates, and any special requests or amenities. Prioritize user comfort, cost efficiency, and proximity to attractions or business centers.
